package Pruebas_de_concepto.backend.codigo.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "codigo-ear.ear");

        // Current directory is the root of the "codigo-web" project. Go up one level
        // and enter the "codigo-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../codigo-ejb/target/codigo-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "codigo-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/codigo-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "codigo-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("Pruebas_de_concepto.backend.codigo.test");
